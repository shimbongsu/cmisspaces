...
package org.httpclient {
  ...
  public class HttpSocket {
    ...
    public function close():void {
      //trace("HttpSocket close()");
      if (_closed) return;
      _closed = true;
      Log.debug("Called close");      
      _timer.stop();
      // sreiner add in this change from new version of lib 3/3/09
      // to not do flush, but kept check on _socket connected 
      // was getting Error #2002: Operation attempted on invalid socket.
      //if (_socket) {
    ...
    private function onResponseComplete(response:HttpResponse):void {
      //trace("HttpSocket onResponseComplete()");
    ...
    private function onRequestComplete(request:HttpRequest, header:String):void {      
      //trace("HttpSocket onRequestComplete()");
    ...
    private function onResponseHeader(response:HttpResponse):void {
      //trace("HttpSocket onResponseHeader()");
    ...
    private function onIOError(event:IOErrorEvent):void { 
      //trace("HttpSocket onIOError()");
      if (_closed) return;      
      close();
      //sreiner _dispatcher.dispatchEvent(event.clone());
    }

    private function onSecurityError(event:SecurityErrorEvent):void {
      //trace("HttpSocket onSecurityError() and on socket timeout");
      if (_closed) return;      
      close();
      //sreiner _dispatcher.dispatchEvent(event.clone());
    }
